...
Page({
  ...
  submit(event){
    ...
    pinghe=pinghe.toFixed(2);
    qixu=qixu.toFixed(2);
    yangxu=yangxu.toFixed(2);
    yinxu=yinxu.toFixed(2);
    tanshi=tanshi.toFixed(2);
    shire=shire.toFixed(2);
    xueyu=xueyu.toFixed(2);
    qiyu=qiyu.toFixed(2);
    tebing=tebing.toFixed(2);
    ...
    let score=[pinghe,qixu,yangxu,yinxu,tanshi,shire,xueyu,qiyu,tebing];
    wx.setStorage({
      // key作为引用时的依据
      key: "Score",
      //将subOrder中数据提交
    ...
    wx.request({
      ...
      success:function (result) {
       ...
       if(result.data.status=="success"){
        ...
        getApp().globalData.text=result.data.data
        wx.redirectTo({
